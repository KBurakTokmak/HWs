...
std::cout << min << "  u.x  " << u.x << " u.y " << u.y << "  u.z "<< u.z << '\n';
...
{
	Matrix4 Mcamera;
std::cout << " u "<< camera.u <<" v  " << camera.v << " w " << camera.w << " gaze "<< camera.gaze << '\n';
...
void Scene::draw(int x,int y, Color color, Camera &camera )
{
	...
	//	std::cout << x<<" "<< y <<"   " << (int)color.r << "  " << (int)color.g << "  "<< (int)color.b << '\n';
		if(color.r > 255)
			image[x][y].r = 255;
		else if(color.r < 0)
			image[x][y].r = 0;
		...
		for(auto &triangle : model->triangles)
		{
			...
			if(cullingEnabled)//backface culling
			{
				for (size_t i = 0; i < model->numberOfTriangles; i++)
				{
						...
						//Vec3 v = subtractVec3(centerOfTriangle , camera->pos);
			...
			if(model->type == 0)/*0 for wireframe,*/
			{
				for(int i = 0; i< model->triangles.size(); i++)
				{
					if(rasterizeIT[i] == 1)
					{
std::cout<<"draw ith triangle "<<i<<std::endl;
						int ver0 = model->triangles[i].vertexIds[0];
						int ver1 = model->triangles[i].vertexIds[1];
						int ver2 = model->triangles[i].vertexIds[2];
						std::cout<< i <<" v0 "<< pixelCoordinates[ver0] <<"  v1 " << pixelCoordinates[ver1] << " v2 " << pixelCoordinates[ver2]  << '\n';
